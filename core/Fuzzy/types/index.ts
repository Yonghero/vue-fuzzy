...
interface Templates {
  ...
  require?: boolean
  placeholder?: string
  items?: Array<{ label: {} | string; value: string | number; type?: number | string }>
  width?: number | string
  visible?: {
    query?: boolean
    table?: boolean
...
interface HandlerParams { data: any; current: any }
...
interface DialogChanageable { text: string; data: any; current: any }
...
export {
  FormItemEnum, OperatorCmd,
}
export type {
  AllModels,
  TemplateConfiguration,
  Templates,
  FuzzyBaseModel, tableOperation,
  Pagination, Api,
  Feature,
  FuzzyHandler,
  RenderDialog,
